...
<?php if($puja_data->pooja_samgri==1 or $puja_data->havan_samagri==1){ ?>
    <a href="<?=base_url();?>pages/Samagri/<?php if($this->uri->segment(3)=='edit'){ $this->uri->segment(4);} else { echo $this->uri->segment(3);} ?>"><span class="btn btn-info btn-lg"> Samagri</span></a>
<?php } ?>
 
<?php if($puja_data->blog_prasads==1){ ?>

						  <a href="<?=base_url();?>pages/BhogPrasad/<?php if($this->uri->segment(3)=='edit'){ $this->uri->segment(4);} else { echo $this->uri->segment(3);} ?>"><span class="btn btn-danger btn-lg"> Bhog / Prasad</span></a>
				...
				<div class="panel panel-default">					
					<div class="panel-body">
						...
						<?php if($this->uri->segment(4)=='edit'){ ?>
							<form action="<?php echo base_url();?>Pages/BhogPrasad/<?=$this->uri->segment(3);?>/edit/<?php echo $this->uri->segment(5); ?>/update" class="" role="form" id="form" method="post" accept-charset="utf-8">
						<?php } else { ?>	
							<form action="<?php echo base_url();?>Pages/BhogPrasad/<?=$this->uri->segment(3);?>/insert" class="" role="form" id="form" method="post" accept-charset="utf-8">	
						<?php } ?>
...
<td>                                                                            
	<a href="<?php echo base_url();?>Pages/BhogPrasad/<?=$this->uri->segment(3);?>/edit/<?php echo $myresult->id; ?>" title="Edit" class="btn-link btn_size has-tooltip text-info" data-placement="top" data-original-title="Edit" onclick="if (confirm('Are you sure?')){return true;}else{event.stopPropagation(); event.preventDefault();};"><i class="fa fa-edit"></i></a>
	
	<a href="<?php echo base_url();?>Pages/BhogPrasad/<?=$this->uri->segment(3);?>/delete/<?php echo $myresult->id; ?>" title="inactive" class="btn-link btn_size has-tooltip text-info" data-placement="top" data-original-title="Edit" onclick="if (confirm('Are you sure?')){return true;}else{event.stopPropagation(); event.preventDefault();};"><i class="fa fa-trash"></i></a>
</td>       
