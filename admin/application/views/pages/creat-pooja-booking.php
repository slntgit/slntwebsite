	...
	<div class="pageheader">
		<div class="media">
			...
			<div class="media-body">
				...
				<h4>Create <?=ucfirst($_GET['type']);?> Booking</h4>
			</div>
		</div><!-- media -->
	</div><!-- pageheader -->
...
<label for="search_service" id="search_string"><?php if(isset($_GET['poojaId'])){ if(isset($_GET['poojaId'])){ echo $qu->description_charge; }?>, <strong>Member Price : <?php if(isset($_GET['poojaId'])){ echo $qu->member_price; }?>, Non-Member Price : <?php if(isset($_GET['poojaId'])){ echo $qu->non_member_price; }}?><strong></strong></strong></label>
                ...
                ?>
<h5 class="itmesssss searchdata" onclick="getSelService(<?=$value->id;?>,'<?=$value->description_charge;?>','<?=$value->member_price;?>','<?=$value->non_member_price;?>','<?=$_GET['type'];?>');">
...
function build_previousMonth($month,$year,$monthString){
 ...
 $type=$_GET['type'];
 return "<div style='width: 33%; display:inline-block;'><a class='btn btn-info' href='?poojaId=".$_GET['poojaId']."&m=" . $prevMonth . "&y=". $prevYear ."&type=". $type ."'><- " . $monthName . "</a></div>";
}
...
function build_nextMonth($month,$year,$monthString){
 ...
 <a  class='btn btn-info'  href='?poojaId=".$_GET['poojaId']."&m=" . $nextMonth . "&y=". $nextYear ."&type=". $type ."'>" . $monthName . " -></a></div>";
}
...
<script>
    $('.day').on('click',function(){
        ...
        var type='<?=$_GET['type'];?>';
        var QueryString ='type=' + type + '&selday='+selday+'&selmonth='+selmonth+'&selyear='+selyear+'&selpooja='+selpooja+'&selnoday='+selnoday;
                    $.ajax({
                      type: "post",
...
<label class="btn btn-warning memstatus" title="1"> Member Price <input type="radio"  name="pooja_amount" id="pooja_amount" value="<?=$qu->member_price;?>" required="" checked=""></label>
<label class="btn btn-warning memstatus" title="0"> Non-Member Price <input type="radio"  name="pooja_amount" id="pooja_amount" value="<?=$qu->non_member_price;?>" required=""></label>

<input type="hidden" name="mem_status" id="mem_status" value="1">
<script type="text/javascript">
  $('.memstatus').on('click',function(){
   var mem_status = $(this).attr('title');
        ...
        <span name="form_err"></span>
					...
					<div class="form-group col-sm-12">
						<div class="col-sm-12 text-left">
							 <label class="control-label required" style="font-weight:700;">Is the booking for self &nbsp; &nbsp;</label> 
...
.head{
 ...
 <script>
                function getSelService(arg,arg2,arg3,arg4,arg5) {
                    // $.ajax({
                    //   type: "post",
                    ...
                    //   }
                    //   });
                window.location.href='<?=base_url();?>pages/CreatPoojaBooking?poojaId='+arg+'&type='+arg5;
    ...
    $('.searchPooja').on('keyup',function(){
        ...
        $.ajax({
            ...
            cache: false,    
            data:'searchkey='+searchcontent+'&type='+type,
            success: function(response){
                $('.searchData').show();
        ...
        function getChangeRadio(arg) {
          //$('#menu_details').html(arg);
              var queryString='menus='+arg;
                $.ajax({
                       type: "post",
                       url: '<?php echo site_url('pages/getCateringDetails');?>',
                       cache: false, 
                       data:queryString,
                       success: function(response){
