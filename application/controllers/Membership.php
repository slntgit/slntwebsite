...
if($this->input->post('member_profile_status')==0){
    $lstuser='ok';
} else {
    $this->db->select('*');
    $this->db->from('slnt_user_membership');
    $this->db->where(array('membership_id'=>$this->input->post('member_id_old')));
    $las= $this->db->get();
    if($las->num_rows()==0){
       $lstuser='ok'; 
    } else {
       $lstuser='ok'; 
		...
		if($lstuser=='no'){
		    $this->session->set_flashdata("member_id_old","<p class='text-danger'>Invalid membership ID</p>");
		} else { 				    
